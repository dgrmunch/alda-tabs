...
(defn t
  "it parses a tab-note to Alda syntax and executes the resulting code"
  [string-fret & [duration]]
  (if (.contains (str string-fret) ".")  (def sf (stri/split (str string-fret) #"\."))
   (def sf [(str string-fret) "0"]))
  (alda-code (str (guitar-note (read-string (nth sf 0)) (read-string(nth sf 1))) duration)))
...
(defn c
  ...
  (alda-code (str   "V1: " (tab-str one duration)    " V2: " (tab-str two duration)
